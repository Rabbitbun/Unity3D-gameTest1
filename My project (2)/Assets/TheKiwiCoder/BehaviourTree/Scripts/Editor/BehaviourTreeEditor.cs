    ...
    public class BehaviourTreeEditor : EditorWindow {
        ...
        void DisplayBlackboard()
        {
            ...
            if (blackboard != null)
            {
                ...
                foreach (var key in blackboard.GetAllNames())
                {
                    var value = blackboard.GetBlackboardData(key);
                    if (value is IBlackboardData)
                    {
                        var dataField = value.GetType().GetProperty("Data");
                        if (dataField != null)
                        {
                            object dataValue = dataField.GetValue(value);

                            EditorGUILayout.BeginHorizontal();
                            EditorGUILayout.LabelField(key, GUILayout.Width(150));
                            DisplayBlackboardValue(dataValue);
            ...
            {
                EditorGUILayout.LabelField("Blackboard is null.");
            }
        }
        void DisplayBlackboardValue(object dataValue)
        {
            if (dataValue is int intValue)
            {
                EditorGUILayout.IntField(intValue);
            }
            else if (dataValue is float floatValue)
            {
                EditorGUILayout.FloatField(floatValue);
            }
            else if (dataValue is string stringValue)
            {
                EditorGUILayout.TextField(stringValue);
            }
            else if (dataValue is bool boolValue)
            {
                EditorGUILayout.Toggle(boolValue);
            }
            else if (dataValue is Vector3 vector3Value)
            {
                EditorGUILayout.Vector3Field("", vector3Value);
            }
            else if (dataValue is Enum)
            {
                EditorGUILayout.EnumPopup((Enum)dataValue);
            }
            else
            {
                EditorGUILayout.LabelField(dataValue?.ToString() ?? "null");
            }
        }
    }
}